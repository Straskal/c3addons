...
class TilemapPolyRenderer {
  ...
  draw(renderer, settings, worldInstances) {
    ...
    if (!tilemapPolySettings.draw) return;
      ...
      for (let j = 0; j < collisionRectArr2d.length; j++) {
        for (let y = 0; y < collisionRectArr2d[j].length; y++) {
          ...
          if (poly !== null) {
            ...
            const points = poly._ptsArr.map((pt, index) =>
              index % 2 == 0 ? rect.getLeft() + pt : rect.getTop() + pt
            );
...
self.TilemapPolyRenderer = TilemapPolyRenderer;
