...
class PathRenderer {
  ...
  draw(renderer, settings, worldInstances) {
    ...
    if (!pathSettings.draw) return;

    const instance = worldInstances
      .filter((inst) => inst.GetUID() === pathSettings.drawForInstance)
      .filter(
        (inst) =>
          inst.GetBehaviorInstanceFromCtor(C3.Behaviors.Pathfinding) !== null
      )
      .map((inst) =>
        inst.GetBehaviorInstanceFromCtor(C3.Behaviors.Pathfinding)
      )[0];
    ...
    const mapCells = instance
      .GetBehavior()
      ._mapState.get(`${cellSize},${cellBorder}`).mapData._cells;
...
self.PathRenderer = PathRenderer;
